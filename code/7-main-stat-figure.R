...
# Read Data --------------------------------------------------------------------
all_stat <- readRDS(file.path(data.dir, "all_stat.Rds"))

# Build Data -------------------------------------------------------------------
data <- all_stat %>% 
  mutate(stat_long = recode_factor(stat,
                                   # Climate 
                                   "climate_plan" = "Detailed climate change action plan",
                                   "climate_some" = "Explicit climate change strategy",
                                   "climate_mention" = "Mentions climate change",
                                   # Objectives
                                   "obj_cc_any" = "Explicit climate change objective",
                                   "obj_cons_both" = "Both species- and unit-level objectives",
                                   "obj_mgmt_lt" = "Long-term management objective",
                                   "obj_cons_any" = "Any conservation objective",
                                   "obj_mgmt_any" = "Any management objective",
                                   # Assessments
                                   "assess_cc_both" = "Explicit climate change assessment ",
                                   "assess_any_both" = "Any threat assessment",
                                   "assess_threat" = "Identified potential threat or stressor",
                                   # Design
                                   "design_cc_all" = "Design considers climate change",
                                   "design_cc_adaptive" = "Adaptive design connected to climate change",
                                   "design_adaptive_any" = "Any adaptive design",
                                   "design_resilience_all" = "Design includes resilience principle",
                                   "design_any" = "Provides design criteria",
                                   # Monitoring
                                   ...
                                   "threat_any_cc" = "Threat reduction strategy linked to climate change",
                                   "threat_any_strat" = "Strategy to address interacting threat",
                                   "mgmt_adapt" = "Discusses adaptive management"))
...
ggplot(data = data %>% filter(figure == 1), aes(x = pct, y = stat_long)) +
  geom_col(aes(fill = type)) +
  geom_text(aes(label = pct, color = type), 
  ...
  scale_fill_manual(values = c("#1155CC", "#6d9eeb", "#c9daf8"),
                    guide = guide_legend(reverse = TRUE)) +
  scale_color_manual(values = c("white", "white", "grey35")) +
  labs(x = "Percent of management plans", y = NULL, fill = NULL) +
  theme_bw() +
  my_theme
  

ggsave(file.path("figs", "Fig3_Stats.png"), width = 6.5, height = 8, dpi = 600)
