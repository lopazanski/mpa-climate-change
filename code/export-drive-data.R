...
extra_list <- read_sheet("https://docs.google.com/spreadsheets/d/1kARXHqNtNaDU1E3UxkspOThLWDzuIKaSHqjtFjcBH30/edit#gid=930877353",
                         sheet = "extra-op6", trim_ws = T)

## Create Simplified Version ----
orig_simple <- orig_list %>% select(mpa_id, search, plan_id, notes, name)
extra_simple <- extra_list %>% select(mpa_id, search, plan_id, notes, name)
...
# Transpose
review <- as.data.frame(t(review)) %>% 
  filter(Q_1 != "name") # drop old id row
...
saveRDS(data, file = file.path(data.dir, "exported-doc-review.Rds"))

## Export question key ----
saveRDS(q_key, file = file.path(data.dir, "exported-question-key.Rds"))
