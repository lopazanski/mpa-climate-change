...
The emLab Google Drive has a few copies of the dataset, here is the link to the more recent version used: https://drive.google.com/drive/folders/12nyiEQ5rlKooNE9Oa99DAy5KFDgZjhzg?usp=sharing
...
# I settled on this list for my search because it seemed reasonable (e.g. included areas seemed to fit the inclusion criteria
# and there weren't any glaring missing pieces from a prior knowledge/geographic standpoint) and it was also a number that 
# was reasonably close to the number of "fully-highly protected" areas on the MPA Atlas Database website (at the time ~1200ish)
...
```{r filter new database}
# Filter new version of MPA Atlas Database
new_data <- atlas2_df %>% #20933
  filter(implemente == 1) %>%  # 1. Implemented (step matches old version) 17793 
  filter(rep_m_area != 0 | calc_m_are != 0) %>% # 2. Coastal or Marine (no category in updated version) 9196
  filter(no_take %in% c("Part", "All") | no_take_ar != 0) # 3. No-take ... 1180?
  
# The new version diverges substantially as soon as the categories no longer match - filtering for "coastal or marine"
# in the original version drops the observations from 13k to 11k, but attempting to replicate the same step in the new
# version drops the observations from 17k to 9k. 

# The no-take step (#3) is significantly different from the previous version because there is no "fishing" category.

# Next steps:
# Reach out to MPA Atlas folks to get clarification on how to filter to get the list of fully/highly protected areas
# (email sent on July 22 2021) or just get the list. Don't waste more time examining what doesn't match; it's several
# hundred cases on each side, and at least 100 cases where management plans were found.
