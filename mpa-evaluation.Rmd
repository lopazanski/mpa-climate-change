...
# Compare the mpa_id's for the fully/highly list to the old search list
fh_id <- unique(fh$mpa_id) # Unique mpa_id's for the fully/highly list
search_list <- read_csv("list-17jan2021.csv") # search list

# Are any of the areas "found" not included in the new list?
old_found <- search_list %>% 
  filter(search == "F") 

found_now_excluded <- old_found %>% # 116 originally included zones are now excluded
  filter(!(mpa_id %in% fh_id))

found_still_included <- old_found %>% # 367 zones "found" still included
  filter(mpa_id %in% fh_id)

mp_included <- unique(found_still_included$plan_id)
all_mp_id <- data.frame("mpa_id" = c(1:153))

mp_excluded <- all_mp_id %>% # 54 management plans now not included
  filter(!(mpa_id %in% mp_included))

# Since 54 already-reviewed plans are excluded, this may be too restrictive for our purposes
```
...
overlap2_plans <- data.frame("plan_id" = as.numeric(unique(overlap2$plan_id[overlap2$search == "F"]))) 

# Extra areas not in original search
extra2<- option2 %>% 
  filter(!(mpa_id %in% search_list$mpa_id)) #388
